...
def _submit_one_row(client, uid: int, row: dict, account_email: str, category: str,
    ...
    try:
        ...
        db.set_pending_job(jid, uid, category, segment_label, row_variant, watermark, product_id)
...
def submit_brief_segments(uid: int, brief: dict, segment_ids: set[str]) -> list[dict]:
    ...
    # Submit every selected segment in parallel (not chained frame-to-frame):
    # useapi.net rejects combining a start-frame image with a reference image in
    # the same request, which made real frame-chaining unreliable and slower for
    # uncertain benefit. Continuity instead relies on the master prompt templates'
    # own "continuing the same X..." language plus the shared reference image.
    results = []
    for row in rows:
